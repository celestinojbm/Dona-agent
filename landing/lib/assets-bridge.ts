// landing/lib/assets-bridge.ts — Bridge HMAC para la Galería de Activos (Fase 1)
//
// Mismo patrón que automation-bridge.ts · firma con
// HMAC-SHA256(body, INTERNAL_BRIDGE_SECRET) y llama /internal/assets en el
// backend Render.
//
// Reglas:
//   - subscription_id viene SIEMPRE de auth() server-side (T1.4.D).
//   - INTERNAL_BRIDGE_SECRET nunca llega al cliente.
//   - El backend resuelve telefono desde subscription_id y sólo devuelve
//     activos de ese telefono (anti-IDOR).
...
/**
 * Consulta los activos generados del usuario en el backend (read-only).
 *
...
export async function fetchAssets(
  ...
  if (!backendUrl) {
    console.error("[BRIDGE-ASSETS] BACKEND_URL no configurada");
    return { ok: false, error: "backend_url_missing" };
  }
  if (!secret) {
    console.error("[BRIDGE-ASSETS] INTERNAL_BRIDGE_SECRET no configurada");
    return { ok: false, error: "internal_bridge_secret_missing" };
  }

  const payload: Record<string, unknown> = { subscription_id: subscriptionId };
  if (opts?.tipo) payload.tipo = opts.tipo;
  if (typeof opts?.limite === "number") payload.limite = opts.limite;
  ...
  const url = `${backendUrl.replace(/\/+$/, "")}/internal/assets`;
  ...
  try {
    ...
    console.error(
      `[BRIDGE-ASSETS] rid=${reqId} status=${res.status} sub=${subShort}`,
    );
    ...
    if (isAbort) {
      console.error(
        `[BRIDGE-ASSETS] rid=${reqId} timeout (${BRIDGE_TIMEOUT_MS}ms) sub=${subShort}`,
      );
      return { ok: false, error: "timeout" };
    }
    const msg = err instanceof Error ? err.message : "unknown";
    console.error(`[BRIDGE-ASSETS] rid=${reqId} fetch falló: ${msg}`);
